...
Blood-based molecular readouts are therefore attractive for AD research because peripheral sampling is comparatively scalable, repeatable, and compatible with large cohorts. Whole-blood or peripheral-blood transcriptomic profiles are one such readout. They may capture immune activation, inflammatory tone, metabolic state, cellular composition, and systemic physiological changes that accompany neurodegenerative disease. Earlier studies reported candidate blood-expression classifiers or disease-associated transcriptional patterns in AD and MCI, including AddNeuroMed-based work that supported the possibility of blood RNA signatures for cognitive impairment or early AD detection [3,4]. These findings helped establish peripheral blood transcriptomics as a plausible exploratory biomarker domain.
...
The primary discovery dataset was GSE63060, an AddNeuroMed peripheral blood microarray dataset measured on GPL6947 Illumina HumanHT-12 V3.0 expression beadchip [5]. The locked AD-versus-control analysis included 145 AD cases and 104 controls.

The related validation dataset was GSE63061, an AddNeuroMed peripheral blood microarray dataset measured on GPL10558 Illumina HumanHT-12 V4.0 expression beadchip [6]. The locked AD-versus-control analysis included 139 AD cases and 134 controls.

The first cross-platform validation dataset was GSE85426, a peripheral blood microarray dataset measured on GPL14550 Agilent-028004 SurePrint G3 Human GE 8x60K Microarray [7]. The locked AD-versus-control analysis included 90 AD cases and 90 controls.

The second cross-platform validation dataset was GSE140829, a large peripheral blood expression dataset measured on GPL15988 HumanHT-12 v4 Expression BeadChip [8]. The analyzable normalized expression subset contained 198 AD cases, 229 controls, and 124 MCI samples. AD-versus-control discrimination was the primary endpoint, while MCI score placement was retained as exploratory context only.
...
Differential expression was performed in GSE63060 only. Linear models were fitted using limma with diagnosis as the main coefficient and age and sex as covariates when available [10]. Candidate genes were selected using Benjamini-Hochberg adjusted P < 0.05 and absolute log fold-change >= 0.20. The main discovery output contained 623 candidate probes, 546 annotated candidate probes, and 476 unique genes. GSE63061, GSE85426, and GSE140829 were not used to select discovery candidates.
...
The main modeling analyses used frozen discovery and validation roles. GSE63060 was used for training, GSE63061 for model selection or development evaluation, and GSE85426 for final stress testing. Primary models included a signed mean gene-only signature and an integrated penalized model including gene features plus age and sex where available. Penalized models were fitted with glmnet [11]. A train-development refit model was also evaluated by refitting on GSE63060 plus GSE63061 without using GSE85426 or GSE140829 labels for tuning. After GSE140829 was frozen as a new large validation cohort, the same frozen or pre-registered models were evaluated in GSE140829 without label-driven optimization.

The prespecified diagnostic-performance threshold was AUC >= 0.70 in an independent validation cohort. Models that did not meet this threshold were not interpreted as supporting clinical use.
...
All datasets analyzed in this study are publicly available from GEO under accessions GSE63060, GSE63061, GSE85426, and GSE140829. Processed audit outputs, intermediate result tables, and scripts are stored in the local project directory.

## Code Availability

The analysis scripts are available in the project `scripts/` directory. Main outputs are stored in `results/`, figures in `figures/route_b/`, and audit reports in `audit/`.

## Ethics Statement

This study reanalyzed publicly available, de-identified GEO datasets. No new human participants were recruited, and no new biological specimens were collected.

## Author Contributions

TODO: Add author initials and contribution roles before submission.

## Funding

TODO: Add funding statement or state that no specific funding was received.
...
3. Lunnon K, Sattlecker M, Furney SJ, Coppola G, Simmons A, Proitsi P, et al. A blood gene expression marker of early Alzheimer's disease. Journal of Alzheimer's Disease. 2013;33(3):737-753. doi:10.3233/JAD-2012-121363.
4. Sood S, Gallagher IJ, Lunnon K, Rullman E, Keohane A, Crossland H, et al. A novel multi-tissue RNA diagnostic of healthy ageing relates to cognitive health status. Genome Biology. 2015;16:185. doi:10.1186/s13059-015-0750-x.
5. NCBI Gene Expression Omnibus. GSE63060: Alzheimer, MCI and control samples from AddneuroMed Cohort (batch 1). Available from: https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE63060.
6. NCBI Gene Expression Omnibus. GSE63061: Alzheimer, Mild Cognitive impairment and control samples from AddneuroMed Cohort (batch 2). Available from: https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE63061.
7. NCBI Gene Expression Omnibus. GSE85426: Peripheral blood gene expression as a biomarker for early detection of Alzheimer's disease. Available from: https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE85426.
8. NCBI Gene Expression Omnibus. GSE140829: Systems-level analysis of peripheral blood gene expression in dementia patients reveals an innate immune response shared across multiple disorders. Available from: https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE140829.
9. Collins GS, Reitsma JB, Altman DG, Moons KGM. Transparent Reporting of a multivariable prediction model for Individual Prognosis Or Diagnosis (TRIPOD): The TRIPOD Statement. Annals of Internal Medicine. 2015;162(1):55-63. doi:10.7326/M14-0697.
10. Ritchie ME, Phipson B, Wu D, Hu Y, Law CW, Shi W, et al. limma powers differential expression analyses for RNA-sequencing and microarray studies. Nucleic Acids Research. 2015;43(7):e47. doi:10.1093/nar/gkv007.
11. Friedman J, Hastie T, Tibshirani R. Regularization paths for generalized linear models via coordinate descent. Journal of Statistical Software. 2010;33(1):1-22. doi:10.18637/jss.v033.i01.
12. Robin X, Turck N, Hainard A, Tiberti N, Lisacek F, Sanchez J-C, et al. pROC: an open-source package for R and S+ to analyze and compare ROC curves. BMC Bioinformatics. 2011;12:77. doi:10.1186/1471-2105-12-77.
...
14. Wu T, Hu E, Xu S, Chen M, Guo P, Dai Z, et al. clusterProfiler 4.0: A universal enrichment tool for interpreting omics data. The Innovation. 2021;2(3):100141. doi:10.1016/j.xinn.2021.100141.
15. Yu G, He Q-Y. ReactomePA: an R/Bioconductor package for reactome pathway analysis and visualization. Molecular BioSystems. 2016;12(2):477-479. doi:10.1039/C5MB00663E.
16. Milacic M, Beavers D, Conley P, Gong C, Gillespie M, Griss J, et al. The Reactome Pathway Knowledgebase 2024. Nucleic Acids Research. 2024;52(D1):D672-D678. doi:10.1093/nar/gkad1025.
