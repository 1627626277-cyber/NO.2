...
Alzheimer's disease is a major neurodegenerative disorder and a leading cause of dementia. Although molecular and imaging biomarkers have improved biological understanding and diagnostic classification, scalable peripheral biomarkers remain attractive because blood sampling is less invasive, more accessible, and more feasible for large cohorts than cerebrospinal fluid collection or advanced imaging. Blood transcriptomic profiles offer one possible route to such markers because they capture immune, inflammatory, metabolic, and systemic physiological variation that may accompany neurodegenerative disease.

Public transcriptomic repositories have enabled many AD biomarker studies. The Gene Expression Omnibus (GEO) contains multiple blood microarray datasets that include AD cases, mild cognitive impairment (MCI) samples, and controls. The AddNeuroMed blood expression datasets GSE63060 and GSE63061 have been used frequently as discovery and validation resources, while additional datasets such as GSE85426 and GSE140829 provide opportunities to evaluate cross-platform reproducibility. These public datasets make it possible to test whether a candidate signature is transferable outside its discovery setting.

However, public-data biomarker studies are vulnerable to optimistic conclusions when validation is incomplete or when model selection uses information from external datasets. A signature that performs well in a related cohort may reflect shared recruitment, processing, platform, or batch characteristics rather than a transferable biological signal. Conversely, a signal that weakens in an independent cohort may still be scientifically useful if the failure is transparently documented and interpreted as evidence of cohort dependence. For blood transcriptomics, this distinction is especially important because cell composition, medication exposure, comorbidity, platform chemistry, and annotation differences can all influence measured expression.

We therefore designed this study as a transportability audit rather than as a search for a high-performance diagnostic classifier. The analysis used GSE63060 for discovery, GSE63061 for related-cohort validation, and GSE85426 plus GSE140829 for cross-platform validation. The core question was not whether a model could be tuned until it exceeded an AUC threshold in a known validation set, but whether a discovery-derived blood transcriptional signal retained direction, statistical support, and diagnostic discrimination across increasingly independent validation contexts.

The study had four objectives. First, we quantified candidate-gene reproducibility across validation cohorts using mapping rate, direction concordance, nominal replication, FDR-level replication, and gene-level oriented AUC. Second, we evaluated whether simple and penalized diagnostic models selected without using the final validation labels retained performance across platforms. Third, we separated stable transportable genes from unstable genes that reversed direction or lost cross-platform signal. Fourth, we tested whether stable and unstable gene sets had coherent pathway context. This framing allows model failure to be treated as a central external-validation finding rather than as an inconvenient negative result.
...
The primary discovery dataset was GSE63060, an AddNeuroMed peripheral blood microarray dataset measured on GPL6947 Illumina HumanHT-12 V3.0 expression beadchip. The locked AD-versus-control analysis included 145 AD cases and 104 controls.

The related validation dataset was GSE63061, an AddNeuroMed peripheral blood microarray dataset measured on GPL10558 Illumina HumanHT-12 V4.0 expression beadchip. The locked AD-versus-control analysis included 139 AD cases and 134 controls.

The first cross-platform validation dataset was GSE85426, a peripheral blood microarray dataset measured on GPL14550 Agilent-028004 SurePrint G3 Human GE 8x60K Microarray. The locked AD-versus-control analysis included 90 AD cases and 90 controls.

The second cross-platform validation dataset was GSE140829, a large peripheral blood expression dataset measured on GPL15988 HumanHT-12 v4 Expression BeadChip. The analyzable normalized expression subset contained 198 AD cases, 229 controls, and 124 MCI samples. AD-versus-control discrimination was the primary endpoint, while MCI score placement was retained as exploratory context only.
...
The main modeling analyses used frozen discovery and validation roles. GSE63060 was used for training, GSE63061 for model selection or development evaluation, and GSE85426 for final stress testing. Primary models included a signed mean gene-only signature and an integrated penalized model including gene features plus age and sex where available. A train-development refit model was also evaluated by refitting on GSE63060 plus GSE63061 without using GSE85426 or GSE140829 labels for tuning. After GSE140829 was frozen as a new large validation cohort, the same frozen or pre-registered models were evaluated in GSE140829 without label-driven optimization.
...
Pathway over-representation analysis was performed separately for strict stable, directional stable sensitivity, and unstable gene sets. The main background was the full S3 candidate gene universe, because the biological question was whether transportable genes differed from the pool of discovery-derived AD blood transcriptional candidates. GO Biological Process, KEGG, and Reactome analyses were performed with Benjamini-Hochberg FDR control. Enrichment results were used as biological context and not as causal evidence.

### Statistical analysis and reproducibility

AUC values were calculated using rank-based or ROC-based methods depending on the analysis stage. Differential-expression P values were adjusted by the Benjamini-Hochberg method. All analysis scripts were written in R and are available in the project `scripts/` directory. Main outputs are stored in `results/`, figures in `figures/route_b/`, and audit reports in `audit/`.
...
1. GEO accession GSE63060. Alzheimer, MCI and control samples from AddNeuroMed Cohort, batch 1. NCBI GEO.
2. GEO accession GSE63061. Alzheimer, Mild Cognitive impairment and control samples from AddNeuroMed Cohort, batch 2. NCBI GEO.
3. GEO accession GSE85426. Peripheral blood gene expression as a biomarker for early detection of Alzheimer's disease. NCBI GEO.
4. GEO accession GSE140829. Systems-level analysis of peripheral blood gene expression in dementia patients reveals an innate immune response shared across multiple disorders. NCBI GEO.
...
6. Friedman J, Hastie T, Tibshirani R. Regularization paths for generalized linear models via coordinate descent. Journal of Statistical Software. 2010;33(1):1-22.
...
9. Wu T, Hu E, Xu S, et al. clusterProfiler 4.0: a universal enrichment tool for interpreting omics data. Innovation. 2021.
10. Reactome Pathway Knowledgebase 2024. Nucleic Acids Research.
