...
gse140829_replication <- data.table(
  ...
  validation_role = "large_independent_external_validation",
...
dataset_role_summary <- data.table(
  ...
  route_b_role = c("discovery_training", "related_external_validation", "cross_platform_stress_test", "large_independent_external_validation"),
...
for (role in c("primary_gene_only", "primary_integrated_or_clinical")) {
  ...
  model_auc_rows[[paste0(role, "_gse140829")]] <- data.table(
    ...
    covariate_mode = row$covariate_mode,
    dataset_accession = "GSE140829",
    evaluation_context = "sequential_external_validation",
...
p_auc <- ggplot(plot_auc, aes(x = dataset_accession, y = auc, group = model_role, color = model_role)) +
  ...
  labs(
    title = "External validation audit: AUC decays across external cohorts",
    x = "Dataset",
    y = "AUC",
    color = "Model role"
  ) +
...
p_metrics <- ggplot(metric_long, aes(x = dataset_accession, y = value, fill = metric)) +
  ...
  labs(
    title = "External validation audit: candidate mapping and directional concordance",
    x = "External dataset",
    y = "Fraction",
    fill = "Metric"
  ) +
...
p_counts <- ggplot(count_long, aes(x = dataset_accession, y = count, fill = metric)) +
  geom_col(position = position_dodge(width = 0.72), width = 0.65) +
  labs(
    title = "External validation audit: nominal/FDR replication declines in independent validation cohorts",
    x = "External dataset",
...
p_auc_dist <- ggplot(candidate_auc, aes(x = dataset_accession, y = auc_oriented, fill = dataset_accession)) +
  ...
  labs(
    title = "External validation audit: single-gene oriented AUC distribution",
    x = "External dataset",
